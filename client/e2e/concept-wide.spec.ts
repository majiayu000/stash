...
type ConceptMarker = 'e' | 'a' | 'b' | 'c' | 'd' | 'f' | 'g' | 'h' | 'i' | 'j' | 'k' | 'l' | 'm' | 'n' | 'o' | 'prd';
...
async function createArea(request: APIRequestContext, name: string): Promise<string> {
  const res = await request.post(`${API}/areas`, { data: { name, description: 'e2e concept-wide seed' } });
...
async function createWorkItem(request: APIRequestContext, projectId: string, title: string): Promise<string> {
  const res = await request.post(`${API}/work-items`, {
    data: { title, projectId, areaId: projectId, kind: 'task', status: 'active' },
  });
  expect(res.ok()).toBeTruthy();
  const body = await res.json();
  const id = body.data?.id ?? body.id;
...
async function seedState(request: APIRequestContext): Promise<SeededState> {
  ...
  return { projectId, workItemId, sessionId: session.id, provider: session.provider };
}
...
function fillRoute(route: string, state: SeededState): string {
  return route
    ...
    .replace(':provider', encodeURIComponent(state.provider))
...
test('ConceptSwitcher clicks through all 16 concept entries', async ({ page, request }) => {
  ...
  for (const entry of [
    ...
    await expect.poll(() => new URL(page.url()).pathname).toBe(fillRoute(entry.route, state));
    await expect(CONCEPT_MARKERS[entry.marker](page)).toBeVisible({ timeout: 10_000 });
  }
});

test('README concept route table resolves, including G deep links', async ({ page, request }) => {
  const state = await seedState(request);
  const readmeRoutes = [
    ['/', 'e'],
    ['/c/a', 'a'],
  ...
  for (const [routeTemplate, markerId] of readmeRoutes) {

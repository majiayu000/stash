...
describe('Work lifecycle grouping', () => {
  ...
  test('uses status and canonical today fields instead of project presence', () => {
    const groups = groupTodosForBoard([
      ...
      todo({ id: 'overdue', status: 'planned', dueAt: '2026-05-13' }),
      todo({ id: 'due-today', status: 'planned', dueAt: '2026-05-14' }),
    ...
    expect(groups.later.map((item) => item.id)).toEqual(['due-today', 'later']);
  ...
  test('moving out of today clears fields that would keep the item in canonical today', () => {
    expect(moveInputForColumn('later')).toMatchObject({
      status: 'planned',
      todayPinned: false,
      scheduledFor: null,
      startAt: null,
  ...
  test('moving into today sends server-relative intent instead of a client date', () => {
    expect(moveInputForColumn('today')).toEqual({
      status: 'planned',
      todayPinned: true,

...
- **UI 策略**：
  ...
  - 如确有需要，未来可在 Work 内增加 Systems 保存视图，但不增加顶层导航
...
1. System 的默认创建入口优先放在哪里：Quick Capture token、SmartLists drawer、还是 Todo Detail 操作区？
...
3. 再评估是否需要 Work 内的 Systems 保存视图。

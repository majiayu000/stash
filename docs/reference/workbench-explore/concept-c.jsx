// concept-c.jsx — Hero + Stream. One project front-and-center as cinematic hero,
// secondary projects below as compact strip, live agent log down the right.
// Different from B: this is "single focus" view, not three-pane.

function ConceptC() {
  const { projects, sessions, todos, stats } = window.AppData;
  const hero = projects[0]; // aurora
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 360px', gap:'1.25rem', flex:1, minHeight:0}}>
          ...
          <div style={{minWidth:0, display:'flex', flexDirection:'column', gap:'1.25rem', overflowY:'auto'}}>
                ...
                <div className="hero-inner">
                  ...
                  <div style={{display:'flex',alignItems:'flex-start',gap:'1.5rem',marginBottom:'1.5rem'}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <h1 style={{fontFamily:'var(--font-mono)',fontSize:'2.6rem',fontWeight:700,letterSpacing:'-0.02em',lineHeight:1.05,marginBottom:'0.4rem'}}>
                        <span style={{background:'var(--gradient-logo)',WebkitBackgroundClip:'text',backgroundClip:'text',WebkitTextFillColor:'transparent'}}>
                          {hero.name}
                        </span>
                      </h1>
                      <div style={{fontFamily:'var(--font-mono)',fontSize:'0.95rem',color:'var(--text-secondary)',marginBottom:'0.8rem'}}>
                        ...
                        <Typewriter
                          phrases={[
                            hero.doing,
                            'patching session.ts (+24 -3)',
                            'running test suite…',
                            '2 tests failing — investigating',
                          ]}
                          speed={45}
                          pause={2200}
                        />
                      ...
                      <div style={{display:'flex',gap:'0.5rem',flexWrap:'wrap'}}>
                        <span style={{fontFamily:'var(--font-mono)',fontSize:'0.75rem',color:'var(--text-muted)',padding:'3px 8px',background:'var(--bg-elevated)',borderRadius:'var(--radius-sm)'}}>⎇ {hero.branch}</span>
                  ...
                  <div style={{marginBottom:'1.25rem'}}>
                    <div style={{display:'flex',justifyContent:'space-between',marginBottom:'0.5rem'}}>
                      ...
                      <span style={{fontFamily:'var(--font-mono)',fontSize:'1.6rem',fontWeight:700,background:'var(--gradient-primary)',WebkitBackgroundClip:'text',backgroundClip:'text',WebkitTextFillColor:'transparent',lineHeight:1,fontVariantNumeric:'tabular-nums'}}>
                        <CountUp to={hero.progress} format={(n)=>Math.round(n)+'%'} />
                      </span>
                    </div>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(5, 1fr)',gap:'0.75rem',marginBottom:'1.25rem'}}>
                    <HeroStat label="tokens · 24h" value={window.fmt.k(hero.tokens24h)} color="var(--neon-cyan)" />
                    <HeroStat label="cost · 24h" value={'$'+hero.cost24h.toFixed(2)} color="var(--neon-green)" />
                    <HeroStat label="sessions" value={hero.sessions} color="var(--neon-purple)" />
                    <HeroStat label="open todos" value={hero.todoCount} color="var(--neon-orange)" />
                    <HeroStat label="last" value={window.fmt.ago(hero.lastTouched)} color="var(--text-secondary)" />
                  </div>
            ...
            <div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(5, 1fr)',gap:'0.75rem'}}>
                {others.map(p => <MiniProjectCard key={p.id} p={p} />)}
              </div>
            </div>
            ...
            <div className="surface">
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.5rem'}}>
                {heroTodos.map(t => <TodoItem key={t.id} t={t} />)}
              </div>
            </div>
          </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'0.75rem',minWidth:0,minHeight:0}}>
            ...
            <div className="live-feed">
              ...
              <FeedLine ts="12:42:18" type="msg"><span style={{color:'var(--neon-cyan)'}}>&gt;</span> proposing Session interface that wraps existing JWT helper</FeedLine>
              ...
              <FeedLine ts="12:42:08" type="info">session:s1 · sonnet-4.5 started</FeedLine>
              <FeedLine ts="12:41:52" type="user"><span style={{color:'var(--neon-green)'}}>$</span> wire oauth callback to session store</FeedLine>
                ...
                <span style={{fontFamily:'var(--font-mono)',fontSize:'0.8rem',color:'var(--text-secondary)',flex:1}}>
                  <Typewriter
                    phrases={[
                      ...
                      'review aurora pr #482',
                      'port lexer fixtures',
                      '#bug auth callback hangs',
                    ]}
                    speed={50}
                    pause={1800}
                  />
...
function FeedLine({ ts, type, children }) {
  return (

// concept-e.jsx — Capture & Plan. Todo-first dashboard.
// Hero: big capture input. Below: 4-column todo board
// (Inbox · Today · Doing · Later). Right rail: project chips
// with "+ new project" CTA. Project execution is secondary here.

function ConceptE() {
  const { projects, todos, sessions } = window.AppData;

  const inbox  = todos.filter(t => !t.project && !t.done);
  // "doing" = todos whose project has a live session
      ...
      <div className="inner" style={{overflow:'hidden', height:'100%'}}>
        ...
        <div className="capture-hero">
          ...
          <div className="capture-hero-inner">
            <div className="capture-row">
              ...
              <span className="capture-kbd">⏎</span>
            </div>
            <div className="capture-hints">
              ...
              <span style={{marginLeft:'auto',color:'var(--text-muted)'}}>
                <CountUp to={todos.filter(t=>!t.done).length} duration={800} /> open · <CountUp to={todos.filter(t=>t.done).length} duration={800} /> done · <CountUp to={inbox.length} duration={800} /> in inbox
              </span>
            </div>
          </div>
        </div>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 290px', gap:'1.25rem', flex:1, minHeight:0}}>
          ...
          <div style={{minWidth:0, display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:'0.85rem', minHeight:0}}>
            <BoardCol
              icon="📥" name="inbox" tone="orange"
              hint="ideas & quick captures"
              items={[...inbox]}
              count={inbox.length}
            />
            <BoardCol
              icon="🌅" name="today" tone="cyan"
              hint="planned for today"
              items={today}
              count={today.length}
            />
            <BoardCol
              icon="🚧" name="doing" tone="green" live
              hint="agent is on it right now"
              items={doing}
              count={doing.length}
            />
            <BoardCol
              icon="📅" name="later" tone="purple"
              hint="this week · someday"
              items={later}
              count={later.length}
            />
          </div>
          ...
          <div style={{display:'flex', flexDirection:'column', gap:'0.75rem', minHeight:0}}>
            ...
            <div className="surface" style={{padding:'0.75rem 0.9rem',marginTop:'auto'}}>
              <div style={{display:'flex',alignItems:'center',gap:'0.4rem',marginBottom:'0.5rem'}}>
                ...
                <span style={{fontFamily:'var(--font-mono)',fontSize:'0.78rem',color:'var(--text-primary)',fontWeight:600}}>2 agents live</span>
              </div>
              <div style={{fontFamily:'var(--font-mono)',fontSize:'0.7rem',color:'var(--text-muted)',lineHeight:1.6}}>
                <div><span style={{color:'var(--neon-cyan)'}}>{'>'}</span> aurora · sonnet-4.5</div>
                <div style={{paddingLeft:14,color:'var(--text-muted)'}}>patching session.ts…</div>
                <div style={{marginTop:6}}><span style={{color:'var(--neon-purple)'}}>$</span> spectre · codex-1</div>
                <div style={{paddingLeft:14,color:'var(--text-muted)'}}>generating event types…</div>
              </div>
            </div>
          </div>
        </div>
...
function BoardCol({ icon, name, tone, hint, items, count, live }) {
  return (
    <div className={`board-col tone-${tone}`}>
...
function ProjectChipRow({ p }) {
  return (
    <div className="proj-chip">
      <span style={{fontSize:'1rem'}}>{p.emoji}</span>
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:6}}>
          <span style={{fontFamily:'var(--font-mono)',fontSize:'0.78rem',color:'var(--neon-cyan)',fontWeight:600,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{p.name}</span>

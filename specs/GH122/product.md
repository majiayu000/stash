...
11. **B-011** 16,384+ cache rows、6,000+ 最近候选和 50,000+ usage events 下，warm 30 日 Burn 必须满足 1 秒预算；三次 warm Worker 在 compact result 返回且执行 GC 后的 JSC heap 均不得超过 32 MiB，进程末次 RSS 相对基线增量不得超过 250 MiB。
...
- [ ] 三次连续 warm 请求记录 Worker post-GC JSC heap 与进程 RSS，验证 Worker heap 每次 ≤32 MiB 且进程末次 RSS 相对基线增量 ≤250 MiB。

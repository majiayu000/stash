...
1. **B-001** 对相同数据、model rates 和查询窗口，Burn 的 totals、cost、daily buckets、hourly heatmap、model mix、project leaderboard、session 去重与 share 必须与现有公开语义精确一致。
2. **B-002** 查询窗口必须在读取 session usage 前生效；窗口外 session 不得读取或解析 `usage_json`。
...
6. **B-006** cache row、JSONL 或 usage 损坏时请求必须显式失败并包含可诊断来源，不得返回零值、部分值或陈旧值冒充成功。
...
11. **B-011** 16,384+ cache rows、6,000+ 最近候选和 50,000+ usage events 下，warm 30 日 Burn 必须满足 1 秒预算；连续 warm 请求不得出现单调 RSS 增长，末次相对基线增量不得超过 250 MiB。
...
- [ ] 三次连续 warm 请求记录 RSS，验证无单调增长且末次相对基线增量 ≤250 MiB。
...
| Cancellation / interruption / partial completion | covered: B-010；Worker 失败清理全部 pending |
